...
- PP-SA-001 and PP-SA-002 initially added URL-shape validation, public-host allowlisting for vendor upstreams, and DNS preflight rejection of private or reserved A/AAAA answers before link-health and vendor-refresh fetches. At the time of this assessment, the recorded remaining limitation was DNS rebinding/TOCTOU between preflight and the actual fetch. Subsequent hardening now carries the approved DNS records into the default no-reuse HTTPS transports through a pinned lookup, closing that resolver window for link-health and vendor-refresh requests. The scripts remain maintainer-side CI/developer controls for repository content, not general-purpose network sandboxes. A lower-risk `npm run check:links:preflight` mode validates URL shape and DNS without issuing HTTP requests.
